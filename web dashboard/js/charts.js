...
const CHARTS = (() => {
    ...
    chartInstances[canvasId] = new Chart(canvas, {
      ...
      options: {
        ...
        scales: {
          ...
          x: {
            ticks: {
              maxTicksLimit: dates.length <= 7 ? 7 : dates.length <= 30 ? 10 : 12,
              maxRotation: 45
            }
          }
        }
  ...
  // ---- SHARED INACTIVITY HELPER ----
  function getInactiveUsers(industryFilter, userFilter) {
    // Simulate inactivity data from dummy user list
    return DATA.users
  ...
  // ---- INACTIVITY RISK TABLE ----
  function renderInactivityRiskTable(industryFilter, userFilter) {
    const container = document.getElementById('churn-risk-table');
    if (!container) return;

    const inactiveUsers = getInactiveUsers(industryFilter, userFilter);

    if (!inactiveUsers.length) {
      container.innerHTML = '<div class="text-muted" style="padding: 20px; text-align: center;">No users with inactivity match current filters</div>';
      return;
    }
  ...
  // ---- INACTIVITY ALERT OVERVIEW (compact, top 5) ----
  function renderInactivityAlertOverview(industryFilter, userFilter) {
    const container = document.getElementById('churn-alert-overview');
    if (!container) return;

    const inactiveUsers = getInactiveUsers(industryFilter, userFilter);
    const top5 = inactiveUsers.slice(0, 5);

    if (!top5.length) {
      container.innerHTML = '<div class="text-muted" style="padding:20px;text-align:center;font-size:13px;">No users with inactivity match filters</div>';
      return;
    }
  ...
  function renderAll(period, industry, channel, referenceDate, user) {
    ...
    // Compute inactivity data for current and previous periods
    let inactivityData = null;
    let prevInactivityData = null;
    if (DATA.computeInactivityData) {
      let curStart, curEnd;
      if (isHourly) {
        curStart = latestDate;
        curEnd = latestDate;
      ...
      inactivityData = DATA.computeInactivityData(curStart, curEnd, channel, industry, user);
      ...
      if (period !== 'all') {
        ...
        if (isHr) {
          ...
          const prevDate = DATA.allDates[prevDateIdx] || DATA.allDates[0];
          prevInactivityData = DATA.computeInactivityData(prevDate, prevDate, channel, industry, user);
        } else {
          const days = period === '7d' ? 7 : 30;
          const endDate = new Date(referenceDate);
          const startDate = new Date(endDate);
          ...
          const prevStartDate = new Date(prevEndDate);
          prevStartDate.setDate(prevStartDate.getDate() - days + 1);
          prevInactivityData = DATA.computeInactivityData(
    ...
    if (!hasData) {
      updateKPIs(data || {}, prevData || {}, summary || {}, trend || {}, activeUserCount || 0, prevActiveUserCount || 0, inactivityData, prevInactivityData);
      return;
    }
    ...
    renderInactivityRiskTable(industry, user);
    renderMonoChannelUsers(industry, user);
    renderChannelStability(summary, data);
    renderInactivityAlertOverview(industry, user);
